...
void Board::setTileType(int x, int y, uint8_t type) {
  getTile(x, y)->state =
      (getTile(x, y)->state & ~TYPE_MASK) | (type << TYPE_SHIFT);
}
...
void Board::initBoard(bool showTiles, bool &loseState, int &bombIndex,
...
void Board::triggerLoseState(bool &loseState) {
  loseState = true;
  for (auto tile : tiles) {
    if (getTileType(tile.x, tile.y) == Mine &&
        !getTileFlag(tile.x, tile.y, CLICKED) &&
        !getTileFlag(tile.x, tile.y, FLAGGED)) {
...
void Board::cycleTiles(int x, int y) {
  if (getTileType(x, y) != Empty)
    ...
    for (auto &tile : getNearbyTiles(x, y)) {
      if (!getTileFlag(tile->x, tile->y, CLICKED) &&
          !getTileFlag(tile->x, tile->y, FLAGGED)) {
        setTileFlag(tile->x, tile->y, CLICKED);
