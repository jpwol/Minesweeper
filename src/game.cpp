...
void Game::initGame(int rows, int cols)
{
    ...
    if(!(SDL_Init(SDL_INIT_VIDEO))){
        window = SDL_CreateWindow("Minesweeper", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, wWidth, wHeight, 0);
        if(window == NULL){
...
void Game::update()
{
    ...
    if(mouseY >= 12 && mouseY <= 32 && !bDown){
        ...
        else if(mouseX >= board.cols*32 - 92 && mouseX <= board.cols*32-64){
            signal = Button2;
            if(mouseState == 1)
                handleResize(16, 16, 16);
        }
        else if(mouseX >= board.cols*32 - 64 && mouseX <= board.cols*32-36){
            signal = Button3;
            if(mouseState == 1)
                handleResize(16, 30, 20.625);
        }
        else if(mouseX >= board.cols*32-36 && mouseX <= board.cols*32-8){
            signal = ButtonR;
            if(mouseState == 1)
                board.initBoard(showTiles, loseState, bombIndex, desiredDensity);
        }
    ...
    if(mouseX >= 0 && mouseX < board.cols*32 && mouseY >= 52 && mouseY < board.rows*32+52){
        mX = mouseX/32, mY= (mouseY-52)/32;
        if(mouseState == 1 && !bDown && !board.getTileFlag(mX, mY, CLICKED) && !board.getTileFlag(mX, mY, FLAGGED) && !loseState){
            board.setTileFlag(mX, mY, CLICKED);
            if(board.getTileType(mX, mY) == Mine)
                board.triggerLoseState(loseState);
            board.cycleTiles(mX, mY);
        }
        if(mouseState == 4 && !bDown && !board.getTileFlag(mX, mY, CLICKED) && !loseState){
