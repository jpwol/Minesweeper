...
void TextureManager::hashTexture(const std::string& key, const std::string& path, SDL_Renderer* renderer)
{
...
void TextureManager::loadTextures(SDL_Renderer* renderer)
{
    ...
    hashTexture("tileFlaggedWrong", "../../res/tile/flaggedTileWrongSmall.bmp", renderer);
...
void TextureManager::handleTileRender(SDL_Renderer* renderer, Board& board, bool loseState)
{
    for(const auto tile : board.accessTiles()){
        SDL_Rect rect{tile.x * 32, tile.y * 32 + 52, 32, 32};
        if(!board.getTileFlag(tile.x, tile.y, CLICKED) && !board.getTileFlag(tile.x, tile.y, FLAGGED))
            SDL_RenderCopy(renderer, getTexture("tileFresh"), NULL, &rect);
        else if(!board.getTileFlag(tile.x, tile.y, CLICKED) && board.getTileFlag(tile.x, tile.y, FLAGGED))
            SDL_RenderCopy(renderer, getTexture("tileFlagged"), NULL, &rect);
        if(loseState && board.getTileFlag(tile.x, tile.y, FLAGGED) && board.getTileType(tile.x, tile.y) != Mine)
...
void TextureManager::handleUIRender(SDL_Renderer* renderer, int bombIndex, Signal signal)
{
    SDL_Rect srcrect;
    ...
    SDL_RenderCopy(renderer, getTexture("BombCountBorder"), NULL, &BombCountBorder);
    ...
    if(bombIndex >= 0){
        for(int i = 0; i < 3; i++){
            ...
            SDL_RenderCopy(renderer, getTexture("NumbersTileset"), &srcrect, &dstrect);
            dstrect.x += 20;
        }
    }
    else{
